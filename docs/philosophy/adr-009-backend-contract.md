...
- **단점 2**: template-spring 의 ADR-016 (DTO Mapper 금지) 철학과 정면 충돌. 백엔드는 이미 mapper 를 없앤 상태인데 클라이언트만 도입하면 방향성 불일치.
...
**계약 구현** (클라이언트)
- [`lib/kits/backend_api_kit/api_response.dart`](https://github.com/storkspear/template-flutter/blob/main/lib/kits/backend_api_kit/api_response.dart) — `ApiResponse<T>` · `ApiError` · `PageResponse<T>` (77줄)
- [`lib/kits/backend_api_kit/error_code.dart`](https://github.com/storkspear/template-flutter/blob/main/lib/kits/backend_api_kit/error_code.dart) — 에러 코드 상수 (24줄)
- [`lib/kits/backend_api_kit/api_exception.dart`](https://github.com/storkspear/template-flutter/blob/main/lib/kits/backend_api_kit/api_exception.dart) — `ApiException` + `safeErrorCode` / `safeErrorMessage` (67줄)
...
**ViewModel 에서 사용**
- [`lib/kits/auth_kit/ui/login/login_view_model.dart`](https://github.com/storkspear/template-flutter/blob/main/lib/kits/auth_kit/ui/login/login_view_model.dart) — `safeErrorCode` / `safeErrorMessage` 사용 예
