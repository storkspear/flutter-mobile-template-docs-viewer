...
공통 코드는 한 레포 (`template-flutter`) 에 모두 두되, 각 앱 (파생 레포) 은 `app_kits.yaml` 과 `main.dart` 에서 **자기가 쓸 Kit 만 선언적으로 등록**. 미선언 Kit 은 tree-shaking 으로 최종 바이너리에서 제거돼요.
...
**핵심 구현** (템플릿 원본)
- [`lib/core/kits/app_kit.dart`](https://github.com/storkspear/template-flutter/blob/main/lib/core/kits/app_kit.dart) — `AppKit` 추상 클래스 · `RedirectRule` typedef (55줄)
- [`lib/core/kits/app_kits.dart`](https://github.com/storkspear/template-flutter/blob/main/lib/core/kits/app_kits.dart) — 레지스트리 · 롤백 · 우선순위 정렬 (178줄)
