...
**Status**: Accepted. 현재 유효. 2026-04-24 작성 / 2026-05-07 갱신 (4번째 recipe `social-auth-app.yaml` 편입). `recipes/` 폴더에 4개 YAML 파일 (`local-only-tracker.yaml` · `local-notifier-app.yaml` · `backend-auth-app.yaml` · `social-auth-app.yaml`). 파생 레포가 복사 → `app_kits.yaml` 로 덮어쓰기.
...
- **압력 D 대응**: recipe 를 **대표 유형 단위로만 제공** (현재 4개). 세분화하지 않음. 세부 조정은 파생 레포 몫.
...
**활성 Kit**: 5개  
**특징**: `template-spring` 쌍 운영 전제

#### 4. social-auth-app (백엔드 + 한국 시장 소셜 로그인)

```yaml
# recipes/social-auth-app.yaml 전체
# 4-provider 소셜 로그인 앱 (한국 시장 타겟).
# Apple 은 iOS 에서만 자동 노출되지만 선언은 그대로 둔다.
# Kakao / Naver 는 native SDK 셋업이 필요 — lib/kits/auth_kit/README.md 참고.
app:
...
**대상**: 한국 시장을 우선하는 SNS · 커뮤니티 · 메신저 (Kakao / Naver 로그인이 사실상 기본)  
**활성 Kit**: 6개 (backend_api · auth · notifications · device_info · update · observability)  
**특징**: `backend-auth-app` 의 변형 — Kakao / Naver SDK 추가 + observability_kit 기본 포함. 짝 백엔드 (`template-spring`) 의 social provider 셋업 (`template-spring ADR-013` 등) 과 1:1.
...
| 서버 있고 로그인 필요 (이메일 + Google + Apple) | `backend-auth-app` |
| 서버 있고 한국 시장 소셜 로그인 (Kakao / Naver 포함) | `social-auth-app` |
| 위 어느 것도 아님 | 4개 중 가까운 것 복사 후 커스터마이징 |

### Recipe 조합이 아닌 경우

**유형 간 혼합** (예: "로컬 + 서버 백업 옵션" 앱) 은 4개 recipe 어느 것과도 정확히 안 맞음. 이때:
...
**포인트 1 — recipe 는 대표 유형 단위 (현재 4개)**  
가능한 조합은 수백 가지. 하지만 **대표 유형** 만 제공해요. 세분화하면 recipe 간 drift 관리 부담 (ADR-004 참조). 현재 4개 (로컬 트래커 · 로컬 알림 · 백엔드 인증 · 소셜 로그인 한국 시장) 가 "대부분 유형 커버 + 관리 가능" 의 균형. 시장별 변형 (예: 한국 vs 글로벌 소셜) 이 본질적으로 다른 SDK 셋업을 요구하면 recipe 분리가 정당화돼요.
...
### 교훈 1 — "대표 유형 단위" 의 균형점은 3~5 사이

recipe 를 2개로 줄이면 유형 커버 부족. 6개 이상으로 늘리면 drift 관리 부담. **3~5개** 가 경험상 sweet spot. 본 템플릿은 처음 3개로 출발했다가 한국 시장 소셜 로그인 케이스(`social-auth-app`)가 SDK 셋업 차이로 별도 recipe 가 정당화돼 4번째로 편입했어요. **세분화 임계점 조건**: (a) SDK · 외부 서비스 셋업이 본질적으로 다름, (b) 사용자 (파생 레포 개발자) 가 자주 마주칠 시장.

**교훈**: 샘플 제공은 **3~5개 정도가 관리 가능 상한**. 세분화 욕구가 생길 때 "이건 recipe 분리할 가치가 있나? 파생 레포 커스터마이징으로 충분한가?" 임계점 조건을 먼저 확인.
...
**Recipe 파일**
- [`recipes/local-only-tracker.yaml`](https://github.com/storkspear/template-flutter/blob/main/recipes/local-only-tracker.yaml) — 완전 로컬
- [`recipes/local-notifier-app.yaml`](https://github.com/storkspear/template-flutter/blob/main/recipes/local-notifier-app.yaml) — 로컬 알림
- [`recipes/backend-auth-app.yaml`](https://github.com/storkspear/template-flutter/blob/main/recipes/backend-auth-app.yaml) — 백엔드 연동 (이메일 + Google + Apple)
- [`recipes/social-auth-app.yaml`](https://github.com/storkspear/template-flutter/blob/main/recipes/social-auth-app.yaml) — 한국 시장 소셜 로그인 (Kakao + Naver 포함)

**파생 레포 대상 파일**
- [`app_kits.yaml`](https://github.com/storkspear/template-flutter/blob/main/app_kits.yaml) — 파생 레포가 recipe 복사해 덮어쓰기
- [`lib/main.dart`](https://github.com/storkspear/template-flutter/blob/main/lib/main.dart) — YAML 과 동기화해야 할 실제 코드
...
- [`ADR-019 · 솔로 친화적 운영`](./adr-019-solo-friendly.md) — "대표 유형 단위로만 recipe 제공" 의 근거
