...
파생 레포에서 프로덕션 서비스로 스텁을 교체하거나, 템플릿 기본 내장 kit을 활성화할 때 참고.
...
- **observability_kit처럼 내장된 kit**은 파생 레포에서 DSN/Key만 `--dart-define`으로 주입하면 자동 동작. 구현체 직접 작성 불필요.
- **kit 미내장 영역**(FCM 등)은 파생 레포에서 인터페이스 구현체를 만들고 Provider를 교체:
  - `lib/common/providers.dart`의 Provider 값을 교체
  - 인터페이스 시그니처는 변경 금지 (다른 앱과 호환 유지)
- **비밀키/DSN은 이 템플릿 레포에 커밋 금지.** 파생 레포의 `.env` (gitignore됨) 또는 CI Secrets로 주입.
- 신규 의존성은 신중히 — pubspec.yaml은 모든 파생 레포에 영향. **가능하면 kit으로 격리.**

## 옵트아웃

내장 kit 중 불필요한 것은 제거 가능:
