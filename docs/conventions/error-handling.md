...
네트워크 요청에서 발생하는 에러는 `ErrorInterceptor` → `AuthInterceptor` → `ApiClient` → ViewModel 순으로 처리됩니다. 각 레이어가 명확한 책임을 가지고 있어 ViewModel은 항상 `ApiException` 하나만 처리하면 됩니다.
...
모든 에러의 **첫 번째 관문**입니다. `DioException`을 `ApiException`으로 변환해 `error` 필드에 담아 다음 인터셉터로 전달합니다.
...
`QueuedInterceptor`를 상속해 요청을 **순차 처리**합니다. 동시 요청이 여러 개 401을 받아도 refresh는 1회만 실행됩니다.

**onRequest — 토큰 주입:**
- `extra['skipAuth'] == true`이면 헤더 주입을 건너뜁니다 (`postRaw()` 전용)
- 토큰이 있으면 `Authorization: Bearer {accessToken}` 헤더를 추가합니다
...
refresh 실패 시 재시도하거나 별도 에러로 감싸지 않습니다. ViewModel이 `isUnauthorized`를 확인해 `signOut()` 여부를 결정합니다.

### 3. ApiClient — ApiException throw

`DioException`의 `error` 필드에서 `ApiException`을 꺼내 throw합니다. `ErrorInterceptor`가 변환하지 못한 경우에만 `ApiException.unknown()`을 throw합니다.
...
ViewModel은 항상 `ApiException`만 처리합니다. raw `DioException`이나 `SocketException`은 올라오지 않습니다.
...
토큰 만료는 `AuthInterceptor`가 자동으로 갱신 후 재시도합니다. ViewModel은 신경 쓰지 않아도 됩니다.

refresh 자체가 실패하면 `isUnauthorized`가 `true`인 `ApiException`이 전달됩니다. `AuthKit`이 이를 감지해 자동 로그아웃합니다.
...
서버가 `VALIDATION_ERROR`와 함께 `details` 필드에 실패 필드 정보를 반환합니다.
...
에러 코드를 UI에 노출할 때는 `safeErrorCode` / `safeErrorMessage`를 사용해 raw exception이 노출되지 않도록 합니다. 자세한 내용은 [API Contract — safeErrorCode](api-contract.md#safeerrorcode--safeerrormessage)를 참고하십시오.

---

## 인터셉터 실행 순서

`ApiClient` 생성자에서 인터셉터가 등록된 순서가 곧 실행 순서입니다.
...
**onRequest**: 1 → 2 → 3 순으로 실행됩니다  
**onResponse**: 3 → 2 → 1 순으로 실행됩니다  
**onError**: 3 → 2 → 1 순으로 실행됩니다

> `ErrorInterceptor`(2번)가 `onError`에서 `DioException.error`에 `ApiException`을 담아 전달하면, `AuthInterceptor`(1번)가 401 여부를 판단해 refresh를 시도합니다.
