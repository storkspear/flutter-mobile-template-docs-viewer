...
이 템플릿은 모든 선택적 기능을 **FeatureKit**으로 캡슐화한다. 앱마다 필요한 kit만
설치(install)하면 해당 기능의 Provider/라우트/부팅 단계/리다이렉트가 자동으로 조립된다.

## 왜 Kit인가

- **앱마다 성격이 극단적으로 다르다**: 백엔드+인증 앱, 로컬 전용 앱, 알림 중심 앱, 가계부 앱...
- **모노리식 템플릿은 관리가 어렵다**: 사용하지 않는 auth/network 코드가 섞여서 비대해짐.
- **명시적 조립이 낫다**: `main.dart`의 `AppKits.install([...])` 한 곳에서 앱 성격이 결정됨.
...
`main.dart`에서 `AppKits.install([...])`로 필요한 kit 나열. 순서는 의존성 순:
...
각 kit의 `requires` (코드 의존)와 `redirectPriority` (라우팅 게이트 순서) 한눈에:
...
- **install 순서**: 의존하는 kit이 먼저. 즉 `BackendApiKit() → AuthKit()` 순. 어기면 `AppKits.install`이 `StateError`로 거부.
- **redirect 합성**: `redirectPriority` 오름차순으로 평가. 첫 번째로 redirect를 반환하는 게이트가 승.
- **새 게이트 추가 시**: 기존 1/10/50 사이에 끼우거나 51 이상으로. 1·10·50은 변경 금지 (UI 흐름 깨짐).
- **kit 제거 시**: `requires`로 의존받는 상위 kit이 있으면 같이 제거 (예: BackendApiKit 빼면 AuthKit도).

---

## 선언적 구성 (`app_kits.yaml`)

루트의 `app_kits.yaml`이 **활성 kit 의도의 진실 출처**. `lib/main.dart`의
`AppKits.install([...])`는 그 의도를 코드로 옮긴 것.
...
`app_kits.yaml`과 `lib/main.dart`의 install 리스트는 **사람이 수동으로 일치**시켜야 한다. 한쪽만 수정하면 빌드는 통과하지만 의도와 다른 동작.
...
Status: ISSUES FOUND
```
→ 수정: `app_kits.yaml`에 `backend_api_kit: {}` 추가하고 `main.dart`에도 `BackendApiKit()` 끼워 넣기. (또는 auth_kit 비활성화)
...
이 도구는 **`app_kits.yaml` 자체의 정합성**만 검증한다 (kit 존재 여부, requires 충족).
`lib/main.dart`의 `AppKits.install` 리스트와의 일치는 **검증하지 않는다**. 동기화는 사람의 책임이며, 빠뜨리면:
- yaml에는 있는데 main.dart에 없음 → 해당 kit 라우트/provider 모두 비활성 (앱은 정상 빌드, 동작만 누락)
- main.dart에는 있는데 yaml에 없음 → 실제로는 활성 (선언/실행 불일치, 다음 사람이 헷갈림)

CI에서는 `dart run tool/configure_app.dart --audit`로 yaml 정합성 가드. main.dart 동기화 자동 검증은 향후 확장 예정.

## 레시피

자주 쓰는 조합을 `recipes/`에 사전 정의:
...
Kit은 서로 독립적이어야 한다. 다른 Kit의 Provider를 읽어야 하면 `requires`로 명시.

## Kit README 표준 양식

`lib/kits/{kit}/README.md`는 아래 섹션을 순서대로 포함한다. 파생 레포 개발자의 온보딩
기준(30분 안에 도입 여부 결정)으로 작성한다.
...
위 섹션 중 해당 없는 것은 생략 가능(예: 의존 플랫폼 권한 없을 때). 하지만
**제공 / 사용법 / 제거** 3개는 필수.

## Kit 제거 가이드

개발자가 특정 kit을 빼고 싶을 때의 표준 절차:

1. **선언 비활성화**: `app_kits.yaml`의 해당 `{kit_name}:` 라인을 주석 처리.
2. **조립에서 제거**: `lib/main.dart`의 `AppKits.install([...])`에서 해당 `Kit()` 인스턴스 제거.
3. **사용처 정리**: `features/` 내에서 해당 kit의 Provider/위젯을 import하는 코드를 제거.
4. **검증**: `flutter analyze` + `flutter test` → 그린이면 완료.
5. **디렉토리 처리** (선택): 확실히 불필요하면 `lib/kits/{kit}/` 디렉토리 삭제. 파생 레포 히스토리가
   혼란스러우면 남겨둬도 무방 — `app_kits.yaml`에서 주석 처리된 상태로 보존.

**주의**: `requires` 의존성이 있는 다른 kit(예: AuthKit → BackendApiKit)을 제거하면 해당 상위 kit도 함께 정리해야 한다.
