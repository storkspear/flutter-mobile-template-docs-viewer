...
> README 12단계 + recipe(local-only-tracker)를 따라 짐로그 앱을 실제로 만들면서 발견한 것들.
...
- 증상: `cp recipes/local-only-tracker.yaml app_kits.yaml` → app.name이 "My Tracker"로 변경됨
- 해결: 복사 후 짐로그로 다시 수정
...
- 해결: 짐로그처럼 안 쓸 거면 install에서 빼기
- 별도 가이드 충분히 있음 (`onboarding_kit/README.md`)
...
- 정정: 처음 노트에 "icon X"라고 잘못 적힘 — 실제로는 icon 파라미터 존재

### 6. configure_app.dart는 main.dart 동기화 검증 X
- 증상: yaml에 observability_kit 빠져있어도 main.dart에서 install되어 있으면 Status: OK
- 해결: 사람이 마지막에 yaml ↔ main.dart 비교
- **이미 알려진 한계** — kits.md에 명시됨
...
- 짐로그 같은 로컬 앱은 자체 SettingsScreen 작성해야 함 (기존 settings_screen은 로그아웃/탈퇴 등 인증 메뉴 위주)
- features/home은 NavShellKit 쓰면 자동 우회되어 OK

### 9. .g.dart 빌드 산출물
- `lib/database/app_database.g.dart`는 codegen 결과
- `.gitignore`에 `*.g.dart` 등록 권장 (현재 템플릿은 그렇게 되어 있는지 확인 필요)
...
함정 5개 만나면서 약 20분 추가 소요. 가이드에 미리 명시되어 있다면 절약 가능.
...
✅ Drift + StreamBuilder 조합 — 자동 리빌드, ViewModel 없이도 깔끔
...
⚠️ NavTab/OnboardingKit/EmptyView 시그니처 헷갈림 (각 README 보면 OK, 하지만 자주 보게 됨)
...
- **기능 완성도**: 9.5/10 (recipe + Kit + 공통 위젯이 정확히 의도대로 작동)
- **첫 사용 친화도**: 7.5/10 (함정 9개 — 가이드만 보강하면 9.0 가능)
- **출시까지 시간**: 짐로그 같은 단순 앱 = **2시간 코드 + 1~2일 외부 의존 작업** (호스팅/스토어 등록)
- **재사용성**: 매우 높음 — 다음 앱은 본 가이드 따라가면 함정 회피하며 1시간 내 시작 가능

총평: **템플릿 자체는 매우 견고**. 가이드 보강 (P0 1건 + 튜토리얼 추가)이 가장 큰 ROI.
